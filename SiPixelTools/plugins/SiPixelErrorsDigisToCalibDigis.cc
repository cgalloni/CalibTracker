...
{
  std::cout<< "Running SiPixelErrorsDigisToCalibDigis.cc" <<std::endl;
  ...
  for (digiIter=thePlaquettes->begin(); digiIter!=thePlaquettes->end(); digiIter++)
    {
      ...
      std::map<uint32_t, TH2F*>::iterator mapIterator =  SiPixelErrorsDigisToCalibDigis_2DErrorInformation_.find(detId);
...
//	    std::cout << "This is the beginning of an error 2d histo booking: "<<std::endl;
	    ...
	    temp_ = bookHistoPlaquetteSummary2D(detId, "SiPixelErrorsCalibDigis", "SiPixelErrorsDigisToCalibDigis");
...
TH2F* SiPixelErrorsDigisToCalibDigis::bookHistogram2D(uint32_t detid, std::string name, std::string title, int nchX, double lowX, double highX, int nchY, double lowY, double highY)
{
 std::string hid = theHistogramIdWorker_->setHistoId(name,detid);
...
TH2F* SiPixelErrorsDigisToCalibDigis::bookHistoPlaquetteSummary2D(uint32_t detid, std::string name,std::string title){
...
void SiPixelErrorsDigisToCalibDigis::GetPixelDirectory(uint32_t detID)
{
...
{
  return folderMaker_->setModuleFolder(&myTFileDirMap_,detID,0);
}
